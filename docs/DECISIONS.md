...
**Rationale:** The operator's collaborative relationship with city government requires collaborative framing. City staff adoption more likely if not perceived as hostile. Accountability is natural consequence of transparency — doesn't need to be the pitch.
...
**Rationale:** After 6+ weeks of building, patterns emerged that needed naming. "Scale by default" was already practice (FIPS everywhere, city config registry) but not articulated as a tenet. "Human-unique boundary optimization" captures the bidirectional insight — the system should flag both "this human task could be automated" AND "this automated task needs human review." "Decision velocity" addresses the bottleneck: the operator's attention is the scarcest resource; the system should optimize every human touchpoint for speed. "Richmond is the ideal" resolves the tension between "build for scale" and "build something great" — Richmond quality wins, then we figure out scale. These tenets inform every feature decision via the parking lot priority groups.
...
**Decision:** Every feature is classified as Public (citizens see it immediately), Operator-only (operator validates framing before any citizen sees it), or Graduated (starts operator-only, promoted to public after human review of tone and accuracy). Classification documented in `docs/PARKING-LOT.md` per item.
...
## 2026-06-07: Complete the Anthropic API rails audit (PR #26 D1-A) — keep the kill-switch lift as an operator decision

**Decision:** Finished the cost-observability work PR #26 set as the precondition for re-enabling scheduled API spend, but did NOT re-enable the schedules or clear `RICHMOND_API_BUDGET_LOCK`. Specifically: (1) closed three call-site import gaps (`bio_generator`, `plain_language_summarizer`, `pipelines/enrichments`) so the budget-lock monkey-patch is carried directly, not via import-order transitivity; (2) added explicit batch-API cost logging (`log_batch_cost` / `log_batch_results_cost`) since async batch results bypass the synchronous `Messages.create` gate — wired into the weekly minutes extraction + four manual batch tools; (3) fixed `_detect_caller` so `python foo.py` attributes to `foo`, not `__main__`; (4) built `cost_digest.py` (per call-site / day / model vs cap) and surfaced a compact MTD top-spenders line in the SessionStart brief; (5) emptied the coverage-test allowlist so the "every anthropic caller imports the lock" invariant is enforced per-file. `force=True` audit: clean — PR #26 already fixed the one real hardcode.

**Rationale:** Investigation showed PR #26's headline task ("instrument 32 call sites with `log_api_cost()`") was already largely solved by the follow-up rails PR's centralized monkey-patch — so the real remaining gaps were narrower and sharper: the batch blind spot (the largest, unattended jobs were uninstrumented, making the $5 cap silently undercount — the exact PR #26 failure mode) and the missing digest. Re-enabling the cron schedules incurs ongoing unattended spend and was the operator's explicit halt; per the judgment-boundary catalog that decision stays with the operator. The digest immediately surfaced that MTD is already at $5.14/$5.00 from *local* runs (`netfile_paper_extractor` $6.46, `plain_language_summarizer` $3.01 over the trailing window) — itself decision-relevant context for whether $5 is the right cap before any schedule comes back on.

## 2026-07-05: OD-14 database diet — execute the reversible cuts only

**Decision:** Operator directed the reversible subset of the OD-14 diet (Supabase free tier, 500 MB limit). Executed in migration 121: delete superseded conflict-flag history (85,270 rows, kept all 18,211 current flags), trim city_permits to 2013+ and city_expenditures to FY2024+ (Socrata mirrors, re-fetchable), convert the four embedding sidecars to halfvec(1536) with HNSW rebuilt on halfvec_cosine_ops, drop ~45 never-scanned / single-column city_fips indexes. A verified encrypted backup (db-backup.yml run 28744532419) gates all deletes. Result: 1,251 MB -> 909 MB. Irreversible options — offloading Document Lake raw text (documents / agenda_item_attachments / motions, ~356 MB) and dropping embeddings (~180 MB, kills semantic search) — deferred to a separate operator decision; both together are what actually fits under 500 MB, or Supabase Pro at $25/mo.

**Rationale:** The S28 interest-profiles spec requires permits/licenses data (trim by year, do not drop), so the diet preserved 2013+ permits. Reversibility was the operator''s stated boundary: every cut is recoverable via full re-sync, re-embedding, or the encrypted weekly backup. Two riders for future sessions: (1) superseded flags regrow ~20 MB/month at current rescan cadence — the P1.1a monthly maintenance step must re-prune and VACUUM, or the diet silently regresses; (2) the search RPCs keep their vector(1536) parameter signatures with halfvec casts inside the bodies, so PostgREST callers needed no changes.

## 2026-08-05: Cost-aware frontier-model routing; premium providers earn each call site

**Decision:** Use the official DeepSeek V4 API directly as the default production text-model layer. Route routine classification and bounded transformations to `deepseek-v4-flash` with thinking disabled; route complex, source-grounded extraction and higher-consequence public narratives to `deepseek-v4-pro` with thinking disabled; enable V4 Pro thinking only for call sites that explicitly require multi-step reasoning. Keep `kimi-k3` as an opt-in challenger and `kimi-k2.6` as the preferred optional vision tier. Neither Kimi model is a silent fallback. OpenAI is an eligible task-specific provider—not merely an emergency fallback—when a representative Richmond benchmark shows a quality-per-dollar win. The first chat candidate is `gpt-5.6-luna` for current cost-sensitive frontier work; embeddings remain on `text-embedding-3-small`.

**Enforcement:** Model aliases resolve through one provider-aware client, unknown or retired aliases fail closed, provider credentials never fall through to another provider, and DeepSeek thinking is disabled unless a call site opts in. `gpt-5.6-luna` is explicit opt-in only: it is not a logical-tier default and cannot act as a fallback. Its first production route is limited to failed negated-motion vote explainers, supported by the 2026-08-07 source-grounded benchmark below. Its OpenAI cache-write and long-context rates are accounted for before spend. `gpt-5-nano` remains forbidden because its current snapshot is scheduled to shut down on 2026-12-11. Every routed call remains behind the monthly and per-event spend caps and is logged with the actual returned model. Promoting Kimi or a premium provider to any additional production call site requires a source-grounded benchmark on that task, not a general leaderboard claim.

**Rationale:** DeepSeek V4 Flash and Pro are the current official models, both with 1M-token context and structured-output/tool support. Direct list pricing at decision time is $0.14/$0.28 per million cache-miss input/output tokens for Flash and $0.435/$0.87 for Pro. Kimi K3 is a capable 1M-context frontier model but always reasons and costs materially more, so making it a universal fallback would contradict the operator's cost constraint. OpenAI's August 2026 standard rate makes `gpt-5.6-luna` ($0.20/$1.20 per million input/output tokens) plausible on input-heavy tasks with bounded output; at list price it becomes cheaper than DeepSeek Pro when output is below roughly 71% of input tokens, before cache effects. GPT-5 nano was evaluated but excluded after the official deprecation schedule showed its current snapshot shutting down on 2026-12-11 with Luna as the recommended replacement. No provider is assumed cheaper or better for every workload. This policy keeps a cheap frontier path as the normal path while making provider choice an evidence-based per-task decision.

## 2026-08-07: Route failed negated-motion vote explainers to OpenAI Luna

**Decision:** Keep `deepseek-v4-flash` as the default vote-explainer model. Route only the narrow case where a motion to deny, reject, block, or postpone an item itself fails to `gpt-5.6-luna`. Add an accuracy rule to the shared vote-explainer prompt that forbids treating a failed negated motion as proof that the underlying agenda item was approved or will proceed.

**Evidence:** A source-grounded benchmark used two real Richmond records: the April 28 Liftech contract vote and the April 21 failed motion to deny the Craneway donation. Before the prompt fix, both DeepSeek tiers made unsupported claims that the donation would proceed; Luna correctly said the supplied record did not establish the donation's final disposition. After the accuracy fix, Luna remained the clearest fully correct response. On the fixed-prompt run, Luna averaged $0.0002434 per explainer, cheaper than V4 Pro's $0.0003872 and about $0.000103 more than Flash. The route is therefore reserved for the rare semantic-risk case rather than promoted generally. Full evidence: `docs/audits/2026-08-07-model-routing-benchmark.md`.

**Boundary:** This is an accuracy/credibility fix, not an automatic fallback. Missing OpenAI credentials fail closed, the ordinary vote-explainer path stays on Flash, and additional OpenAI routes still require their own representative Richmond benchmark.

## 2026-08-08: Quarantine Luna vision to image-only Form 460 summaries

**Decision:** Keep Form 460 text extraction on DeepSeek and full image-based contribution extraction on the explicit optional Kimi route. For only the bounded summary-page path of an image-only Form 460, select `gpt-5.6-luna` when its own credential is configured. Render at most six local PNG pages with `detail=original`, require the structured summary tool, disable thinking, enforce the FPPC summary arithmetic, and allow at most one Luna correction pass after a deterministic arithmetic failure. Missing vision credentials or any still-invalid result fails closed and leaves the filing retryable.
