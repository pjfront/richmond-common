...
| P1.2 | ✅ **CIVIC RECURRENCE SHIPPED 2026-08:** `docs/scheduled_civic_events.yaml` has a validated, bounded annual-rule schema with stable occurrence IDs and explicit per-year overrides; annual Form 700 and semiannual Form 460 review reminders; Richmond's official 2026 pre-election dates and Form 497 monitoring window; the PIR-sourced 2027 domain-renewal reminder; and the existing 90-day horizon check. Every active item has an HTTPS source, response mode, and action-first operator instruction. The old PAT-rotation item was removed because in-repo dispatch now uses the short-lived `GITHUB_TOKEN`; external n8n orchestration is retired. **Still open:** add other provider/account infrastructure-expiry dates only when their authoritative account source supplies a real date—do not guess. | [AI] | medium | Kills the "migration 119 re-seed depended on human memory" class without creating false calendar precision. |
