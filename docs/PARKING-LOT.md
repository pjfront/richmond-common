...
- **S23.3 — Topic landing pages** ✅ — `/topics` index with 14 local issue cards + counts. `/topics/[slug]` chronological timeline grouped by meeting date. `getTopicCounts()` and `getTopicItems()` queries. Publication: **Public** (graduated).
- **S23.4 — "Most Discussed" page** ✅ — `/meetings/most-discussed` with narrative framing (D6). Sorted by public comment count, tiebreaker vote split then motions. Uses `getControversialItems()` RPC (migration 084). Publication: **Public** (graduated, renamed from "Most Debated").
