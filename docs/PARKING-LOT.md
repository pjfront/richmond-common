...
| S24.23 | Transcript-based vote extraction (preliminary motions/votes) | Currently per-item vote display reads from `motions`/`votes` tables which are populated only by minutes_extraction (4-6 weeks lag). The transcript_recap text already mentions vote outcomes ("rejected 4-3, with X, Y, Z voting no"). Build an enrichment that uses Claude to parse structured motions+votes from transcripts, writes with `source='transcript'` flag, and gets overwritten by `source='minutes'` once minutes arrive. Frontend shows preliminary badge until verified. |
