...
| B.61 | Public Comment Sentiment Classification & Vote Alignment | A, B, C | S3.1 (summaries), existing `public_comments` table | **Three layers:** (1) **Sentiment classification** — LLM classifies each public comment as `support`, `oppose`, or `neutral` (covers mixed/unclear/procedural). Both written submissions and verbal comments (already extracted from minutes). Migration adds `sentiment VARCHAR(10)` to `public_comments`. Batch API for historical backfill (~11K comments, ~$5-10). Pipeline integration: classify during extraction for new meetings. (2) **Item-level sentiment summary** — aggregate comment sentiment per agenda item: "12 comments: 8 oppose, 3 support, 1 neutral." Display on meeting detail page alongside vote outcome. (3) **Vote-vs-community alignment analysis** — per council member: how often does their vote align with the majority sentiment of public comments? Surface items where council voted opposite to overwhelming public comment direction. Per-member alignment score over time. Frontend: alignment indicator on council profiles, filterable on meeting pages ("show items where council voted against public comment majority"). **Value: Representation** — this is the most direct measurement of whether elected officials are responsive to constituent input. **Publication tier:** Graduated (sentiment classification is analytical; alignment framing needs careful review to avoid adversarial tone — "responsiveness" not "defiance"). Origin: 2026-03-26 operator request. |
| B.62 | Community Comment Submission to Public Record | A, B, C | B.61 (sentiment), S20 (comment pipeline) | **Residents discuss agenda items on Richmond Commons; discussions are batch-submitted to the City Clerk as part of the public record before meeting deadlines.** Threaded community discussion on agenda items with deadline-aware comment windows. Submission pipeline formats threads as structured email to `cityclerkdept@ci.richmond.ca.us`. **Five open decisions:** (1) Consent/disclosure language for commenters, (2) Clerk submission email format (per-item vs per-meeting), (3) Identity requirements (name only vs email verified vs residency signal), (4) Moderation policy (post-publish with flagging recommended), (5) City relationship approach (pilot with clerk recommended). **Reference implementation:** branch `claude/community-comments-submission-khmuM` (migration 068, API route, frontend component — not merged). **Spec:** `docs/specs/community-comments-spec.md`. Existing infrastructure: `submit_comment_to_clerk()` in `comment_generator.py` (SMTP skeleton). **Phasing:** Phase 1 community discussion + manual submission, Phase 2 automated batch submission, Phase 3 notifications + cross-meeting threads. **Publication tier:** Graduated. **Value: Representation** — most direct expression of "respect for people's voice." Origin: 2026-03-28. |
