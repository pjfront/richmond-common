...
| B.45 | Political Influence Cross-Referencing | A, B, C | B.46 (entity resolution), B.44 (regulatory data), S5.1 (Form 700), S8.1 (expenditures) | Systematic cross-referencing of regulatory actions against political connections. Five ranked cross-references (from research): ✅ (1) Campaign donor → contract recipient (14/15, via `signal_donor_vendor_expenditure`), ✅ (2) Form 700 → council vote (13.5/15, via `signal_form700_*`), (3) LLC ownership chain (12/15, **blocked: requires B.46**), (4) City vendor → family/associate (11.5/15, **blocked: requires B.46**), ✅ (5) Donor → permit applicant → decision (11/15, via `signal_permit_donor` + `signal_license_donor`, 2026-03-15). Three of five cross-references now operational. Remaining two require entity resolution (B.46). Research: `docs/research/political-influence-tracing.md`. Origin: 2026-03-07. |
