...
- **Paths:** A, B, C
- **Status:** Prompt v5 finalized (2026-03-20). 5 iterations: complementary headline/summary design, infinitive verb headlines (tenseless), "if approved" conditionals, zero-context reader assumption, staff report integration. Batch API runner built (`batch_summarize.py`). Readability validator built (`validate_summaries.py`). Migrations 045 (summary_headline) + 046 (agenda_item_attachments) run. Attachment backfill pipeline built (`backfill_attachments.py`). Frontend wired: headline as card header, summary in "In Plain English" box, official text demoted. Item numbers and colored significance borders removed from cards. **Remaining:** Scrape attachments for ~235 remaining eSCRIBE meetings, then execute R1 batch (~$40-60).
