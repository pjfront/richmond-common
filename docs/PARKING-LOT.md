...
| S24.20b | ✅ YOUTUBE_COOKIES wired through transcript fetch | Shipped 2026-04-25. Cookies in GitHub secret + Deno + `--remote-components ejs:github` solves YouTube's n-challenge. yt-dlp can now fetch auto-captions from KCRT live-streams as soon as they're available. Plus the owner-only `operator-post-meeting-recap` typed repository event accepts one optional 11-character `video_id` payload when title-based discovery fails (e.g., live-stream titles without dates). |
...
| S28.3 | Organization profile pages (unions, corporations) | Built and previously graduated in PR #73. **Held operator-only and noindex through November T14 by the 2026-08-29 campaign-directory decision.** Underlying data and exports remain available; reconsider publication only after the joined T14 packet, without provenance backfill or broader S26/S28 expansion in this release. |
| S28.4 | PAC pages graduation | Built and previously graduated 2026-07-06. **Held operator-only and noindex through November T14 by the 2026-08-29 campaign-directory decision.** The Contributions menu and treatment-sitemap entries are removed while the data and operator CSV exports remain intact. Do not automatically republish at T14. |
