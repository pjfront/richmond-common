...
| D28 Category Recategorization Pass | Session | **In progress (2026-03-26).** Root cause: `categorize_item()` keyword matcher had ordering bugs — proclamation was a governance keyword, appointments/litigation/proclamation categories missing from list entirely, "amendment" too broad for contracts. Fix: (1) `categorize_item()` rewritten with correct priority ordering (specific before broad). (2) Full LLM recategorization of all 11,645 non-procedural items via Batch API (`batch_recategorize.py`, msgbatch_01G3M3X6K6zqEEFSpqgowwd8). Prompt with 7 disambiguation rules. ~$6 estimated. ~2,400 items expected to change category (~20% of content). Import pending batch completion. Also: homepage copy updated for public launch (stats bar, How It Works, quick links reframed for non-operator features). |
