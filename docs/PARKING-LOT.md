...
- **Publication:** ~~Graduated~~ → ~~Public~~ → Operator-only (reverted 2026-03-07: batch scan validation showed scanner produces high noise / low signal. 21K flags, 1% abstention coverage. Awaiting scanner v2 quality improvements before graduation).
...
| B.45 | Political Influence Cross-Referencing & Entity Registry | A, B, C | B.44 (regulatory data), S5.1 (Form 700), S8.1 (expenditures) | **Research-first item.** Systematic cross-referencing of regulatory actions (permits, licenses, code enforcement) against political connections (contributions, Form 700 disclosures, expenditure vendors). Requires entity resolution across datasets: is the LLC that got a building permit the same entity that donated to a council member? Extends to nonprofit/corporate registries (CA Secretary of State business filings, IRS 990s for nonprofits) as external data sources for ownership/officer cross-referencing. The goal: trace the full influence loop from donation to regulatory outcome using only public records. Each link in the chain is factual; the pattern is the signal. Deep research session output: `docs/research/political-influence-tracing.md`. Origin: 2026-03-07. |
