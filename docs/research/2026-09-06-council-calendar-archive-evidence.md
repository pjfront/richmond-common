...
Validation: 64 targeted Python tests, 14 frontend tests, TypeScript checking and targeted ESLint pass. Regression tests exercise generated image-only PDFs through the actual hashing/writer code, stable-ID replay, distinct-scan identity, historical text hashes, absent/invalid evidence, count and byte budgets, and progress across repeats. Browser checks at desktop and 390px width show English/Spanish notice copy, its official source link, no horizontal overflow, and no framework error overlay. Production execution remains with the release coordinator after review.
