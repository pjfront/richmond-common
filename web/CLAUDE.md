...
4. Creates a temporary upload solely from `git archive <approved-sha>` with Git replacement objects disabled; rejects symlinks and non-example env files; applies the 50 MB/2,000-file caps to that artifact
...
If deployment/alias attestation is ambiguous, the packet must first resolve the actual current production deployment ID and exact full Git SHA; it must not assume the approved SHA went live. If actual current source cannot be proven, the verdict is `UNSAFE/UNKNOWN` and no rollback command is offered.
