...
export default function DistrictSelector({ selectedDistricts, onChange, councilMembers }: DistrictSelectorProps) {
  ...
  return (
    <div>
      <h3 className="text-sm font-semibold text-civic-navy mb-3">Your district</h3>
      <p className="text-xs text-slate-500 mb-3">
        Save your district as local context. This does not filter email delivery.
      </p>
        ...
        {DISTRICTS.map((d) => {
          ...
          return (
            <button
              ...
              aria-label={`Save District ${d}${member ? `, ${member}` : ''}`}
              onClick={() => toggle(d)}
