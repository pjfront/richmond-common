...
const labels: Record<FinanceEvent['event_kind'], string> = {
  receipt: 'Cash contribution reported received', transfer: 'Contribution reported made', independent_expenditure: 'Independent expenditure', refund: 'Refund', loan: 'Loan', noncash: 'Noncash contribution',
}
...
export default async function MoneyLedger({ searchParams }: { searchParams: Promise<{ q?: string | string[]; committee?: string | string[]; page?: string | string[] }> }) {
  ...
  return <article className="mx-auto max-w-4xl px-4 py-10 sm:px-6">
    ...
    <p className="mt-3 leading-relaxed text-slate-600">This index covers available 2026 activity through election day. It includes primary-season reports. An election is shown only when established from the source; a mention of a candidate alone does not identify the election.</p>
    ...
    {!snapshot ? <p role="status" className="mt-8 rounded-lg border border-amber-300 bg-amber-50 p-5">The campaign record index could not be loaded. This does not mean there were no reports. Please try again later.</p> : <>
      ...
      {shown.length ? <ol className="mt-4 divide-y divide-slate-200">{shown.map(row => <li key={row.event_key} className="py-6">
        <div className="flex flex-wrap justify-between gap-2"><h2 className="text-lg font-semibold text-civic-navy">{labels[row.event_kind]}</h2><span className="text-xl font-semibold text-civic-navy">{money(row.amount)}</span></div>
        ...
        <div className="mt-1 flex flex-wrap gap-x-5">{[...new Set(row.source_urls.length ? row.source_urls : [row.source_url])].map((url, index) => <a href={url} key={url} className={linkClass}>Original filing {row.filing_ids[index] ?? index + 1}</a>)}</div>
