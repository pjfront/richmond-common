...
vi.mock('@/lib/supabase-admin', () => ({
  getSupabaseAdmin: () => ({ from: vi.fn(), rpc: mocked.rpc }),
}))
...
describe('POST /api/email/retry-deliveries', () => {
  beforeEach(() => {
    mocked.rpc.mockResolvedValue({ data: 0, error: null })
  })
  ...
  it('surfaces a shared bounded backlog as workflow-detectable failure', async () => {
    ...
    expect(mocked.rpc).toHaveBeenCalledWith('prune_subscription_activations')
  ...
  it('fails closed when the retention rule cannot be enforced', async () => {
    process.env.API_SECRET = 'test-secret'
    mocked.rpc.mockResolvedValue({
      data: null,
      error: { message: 'retention RPC unavailable' },
    })
    const request = {
      headers: new Headers({ authorization: 'Bearer test-secret' }),
    } as unknown as NextRequest
