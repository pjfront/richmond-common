...
describe('POST /api/subscribe credential boundary', () => {
  ...
  it('reactivates as a new welcome cycle without disclosing the manage token', async () => {
    ...
    expect(body).toEqual(publicSuccess)
  ...
  it('keeps a retryable welcome delay private', async () => {
  ...
  it('returns the same public success for an insert race', async () => {
    mocked.from
      .mockReturnValueOnce(query())
      .mockReturnValueOnce(query({
        data: null,
    ...
    const response = await POST(request({ email: 'race@example.test' }))

    expect(response.status).toBe(202)
    await expect(response.json()).resolves.toEqual(publicSuccess)
    expect(mocked.deliverTrackedEmail).not.toHaveBeenCalled()
  })

  it('returns the same public success for a reactivation race', async () => {
    mocked.from
      .mockReturnValueOnce(query({
        data: {
          id: 'subscriber-race',
