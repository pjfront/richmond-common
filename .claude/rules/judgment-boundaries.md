...
- **Source-closest artifact identification.** Before writing or modifying any extractor/generator, identify the persisted source-closest artifact for its input (the rule and reference table live in `.claude/rules/conventions.md`). Before debugging incorrect generated output, identify the input artifact first — most "prompt failures" are actually input-source failures. Updating a generator's docstring to declare its input artifact is AI-delegable. Switching a generator from a derivative input to a source-closest input is AI-delegable when the source-closest artifact is already persisted; if it requires building new persistence infrastructure, surface as a judgment call.
...
- **Production deploy command execution.** Running `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\web\scripts\deploy-prod.ps1 <full-sha>` from the Windows operator checkout is AI-delegable only AFTER the operator approves a decision packet that states the full 40-character commit SHA and complete list of included changes (see judgment call below). The tracked launcher selects Git for Windows explicitly because bare `bash` resolves to unusable WSL on this machine. Approval is bound to that one SHA. The shell gate proves that SHA is clean canonical GitHub `main` with exact green CI, deploys an immutable Git archive through the pinned Richmond Vercel target, and attests the resulting production alias; it cannot prove that human approval happened. If `main` advances, execution stops until a new decision packet and approval name the new SHA. Setup + boundary expansion landed 2026-05-18; before that, deploys were operator-only. Rationale: `web/CLAUDE.md` Deployment Gating section.
...
- **Approving a batch for production deploy.** Whether the changes merged to `main` since the last deploy are ready to ship to richmondcommons.org. The decision packet must state the full 40-character commit SHA, the complete list of included changes since the last production deployment, and the user-visible impact. Approval is valid only for that SHA. If `main` advances for any reason, a new decision packet and approval are required before deployment. After approval, the exact-SHA deploy command itself is AI-delegable per the section above. Companion to the Vercel gate added 2026-05-16: the gate prevents auto-deploy, this rule defines the human-in-the-loop step.
