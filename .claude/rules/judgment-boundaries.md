...
- **Production deploy command execution.** Running `bash web/scripts/deploy-prod.sh` to push the latest `main` commit to richmondcommons.org is AI-delegable, AFTER the operator has OK'd a specific batch (see judgment call below). The script enforces a green Build Check pre-flight and uses env-var-based Vercel linkage from `.env`. Setup + boundary expansion landed 2026-05-18; before that, deploys were operator-only. Rationale: `web/CLAUDE.md` Deployment Gating section.
...
- **Approving a batch for production deploy.** Whether the changes merged to `main` since the last deploy are ready to ship to richmondcommons.org. AI summarizes the impact (which pages change, what users will see differently); operator approves the batch. After approval, the deploy command itself is AI-delegable per the AI-Delegable section above. Companion to the Vercel gate added 2026-05-16: the gate prevents auto-deploy, this rule defines the human-in-the-loop step.
