...
-- Skip any that have child references (public_comments, motions, etc.) to avoid FK violations.
DELETE FROM agenda_items
WHERE item_number ~ '^[A-Z]+$'
  AND LENGTH(item_number) <= 4
  AND id NOT IN (SELECT agenda_item_id FROM public_comments WHERE agenda_item_id IS NOT NULL)
  AND id NOT IN (SELECT agenda_item_id FROM motions WHERE agenda_item_id IS NOT NULL);
