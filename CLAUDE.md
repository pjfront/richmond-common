...
- **Cloud pipeline Phase E — Local pipeline retirement (complete):** Consolidated `cloud-pipeline.yml` to be the single pipeline workflow — added weekly cron schedule (Monday 6am UTC, auto-detects next Tuesday), auto-detect trigger source (scheduled/manual/n8n). Deprecated `sync-pipeline.yml` (which called local `run_pipeline.py`). Production pipeline now fully cloud-native: all data flows through Supabase, no local machine in the loop. `run_pipeline.py` preserved for local development/testing only. Migration health check: `/api/health` endpoint probes 16 tables across 4 migration groups (core schema + migrations 001-003), returns healthy/degraded/unhealthy status. `staleness_monitor.py` gains `check_schema_health()` for CI/pipeline use. 255 total tests.
...
- **Per-city data sources:** `escribemeetings` (base_url), `netfile` (agency_id, agency_shortcut), `nextrequest` (base_url, city_slug), `archive_center` (base_url, minutes_amid), `calaccess` (search_terms for Richmond-area filer matching), `socrata` (domain, datasets dict). Each source has different config fields because each platform's API is different.
...
- **Testing:** `tests/test_city_config.py` (unit tests for registry functions), `tests/test_city_config_integration.py` (integration tests verifying pipeline config resolution), plus per-scraper `tests/test_*_city_config.py` files testing config resolution for each refactored module. 317 total tests.
