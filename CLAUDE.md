...
- **Cloud pipeline Phases B–C (code side):** `src/staleness_monitor.py` — CLI tool querying `data_sync_log` for per-source freshness against configurable thresholds (NetFile 14d, CAL-ACCESS 45d, eSCRIBE 7d, etc.). `web/src/app/api/data-freshness/route.ts` — GET endpoint returning freshness status per source with 1hr cache. Phase C temporal integrity (prospective/retrospective scan modes, contribution date filtering) was already fully implemented in Phase A's `cloud_pipeline.py`. n8n Cloud workflows are external configuration (4 workflows: weekly data sync, monthly CAL-ACCESS, pre-meeting pipeline, retrospective re-scan).
...
1. **Cloud pipeline infrastructure (Phases D–E remaining):** Phase A (core infrastructure) complete. Phase B (n8n orchestration, staleness monitor, data freshness API) complete — n8n Cloud workflows are external configuration, code side done. Phase C (temporal integrity) complete in Phase A. Remaining: Phase D — NextRequest/CPRA document ingestion. Phase E — multi-city orchestration. Spec: `docs/specs/cloud-pipeline-spec.md`.
