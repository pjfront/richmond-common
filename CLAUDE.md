...
- **Cloud pipeline Phase E — Local pipeline retirement (complete):** Consolidated `cloud-pipeline.yml` to be the single pipeline workflow — added weekly cron schedule (Monday 6am UTC, auto-detects next Tuesday), auto-detect trigger source (scheduled/manual/n8n). Deprecated `sync-pipeline.yml` (which called local `run_pipeline.py`). Production pipeline now fully cloud-native: all data flows through Supabase, no local machine in the loop. `run_pipeline.py` preserved for local development/testing only. Migration health check: `/api/health` endpoint probes 18 tables across 5 migration groups (core schema + migrations 001-003, 005), returns healthy/degraded/unhealthy status. `staleness_monitor.py` gains `check_schema_health()` for CI/pipeline use. 255 total tests.
- **Temporal correlation analysis (post-vote donations):** `src/conflict_scanner.py` extended with `scan_temporal_correlations()` — detects donations filed after officials voted favorably on related agenda items. Time-decay confidence scoring (1.0x at 0-90 days → 0.3x at 2-5 years). 5-year configurable lookback window (covers longest commission term). Aye-vote only (Nay correlation deferred pending stakeholder mapping — see `docs/DECISIONS.md` 2026-02-22). Purely factual description format, no judgment language. Integrates into retrospective scan path in `cloud_pipeline.py`. Frontend: `ConflictFlagCard.tsx` shows amber "X days after vote" badge, `reports/[meetingId]/page.tsx` has dedicated "Post-Vote Donations" section. Quarterly retrospective cron in GitHub Actions (`0 8 1 1,4,7,10 *`). CLI: `python conflict_scanner.py <meeting.json> --temporal-correlation --contributions <file>`. Design doc: `docs/plans/2026-02-22-temporal-correlation-design.md`. 278 total tests.
- **Cloud pipeline Phase E — Multi-city orchestration (complete):** `src/city_config.py` — central city configuration registry mapping FIPS codes to per-city data source configs (URLs, API IDs, portal slugs, council member lists). `get_city_config(fips)` resolves config or raises `CityNotConfiguredError`. `list_configured_cities()` returns all registered cities. Richmond config includes 7 data sources (escribemeetings, netfile, nextrequest, archive_center, calaccess, socrata, commissions_escribemeetings) plus council member lists (current + former). All 8 scrapers/clients refactored: `escribemeetings_scraper.py`, `netfile_client.py`, `nextrequest_scraper.py`, `archive_center_discovery.py`, `calaccess_client.py`, `socrata_client.py`, `conflict_scanner.py`, `cloud_pipeline.py`. Each accepts city config instead of module-level constants. Orchestration files (`cloud_pipeline.py`, `data_sync.py`, `staleness_monitor.py`) validate via `get_city_config()` and pass `city_fips` through all calls. Both CLIs gain `--list-cities` flag. 317 total tests including 8 integration tests verifying pipeline config resolution. Plan: `docs/plans/2026-02-22-multi-city-orchestration-plan.md`.
- **Commissions & board members (complete):** Database foundation + data extraction infrastructure for Richmond's 30+ commissions and boards. `src/migrations/005_commissions.sql` — idempotent migration adding `commissions` (name, type, authority, meeting schedule, FIPS) and `commission_members` (term dates, appointing authority, seat type, status, with FK to officials) tables with indexes and RLS policies. `src/ground_truth/officials.json` — seeded with 17 real Richmond commissions (Planning, Rent Board, Design Review, Police, Housing Authority, plus 12 more) scraped from `ci.richmond.ca.us/Boards`. `src/commission_roster_scraper.py` — HTML table-based scraper for Richmond's commission pages, extracts member names, appointing authority, and term dates from `<table>` rows. Pure `requests` + BeautifulSoup (no Playwright). Handles multi-format term dates (MM/DD/YYYY, Month YYYY, "Pleasure of the Mayor"). 29 tests. `src/appointment_extractor.py` — Claude API-powered extractor that mines council meeting minutes for commission appointment actions (new appointments, reappointments, resignations, term expirations). Structured output with commission name, member, action type, vote, and term dates. Includes Richmond-specific appointment patterns (mayoral vs. council district appointments). 22 tests. `src/escribemeetings_scraper.py` extended with `discover_meeting_types()` — catalogs all meeting types from eSCRIBE calendar API across configurable date ranges, with meeting counts and date ranges per type. CLI: `--discover-types [--start-year YYYY] [--end-year YYYY]`. 7 tests. `commissions_escribemeetings` registry added to `city_config.py` mapping canonical commission names to eSCRIBE MeetingName values. Migration health check updated in both `/api/health` route and `staleness_monitor.py` with `005_commissions` group. 403 total tests. Plan: `docs/plans/2026-02-22-commissions-plan.md`. Spec: `docs/specs/commissions-board-members-spec.md`.
...
4. **City leadership & top employees:** ✅ Complete. Socrata payroll ingester, hierarchy classifier, migration 004, health check. 411 total tests. Plan: `docs/plans/2026-02-22-city-leadership-plan.md`. Spec: `docs/specs/city-leadership-spec.md`.
...
6. **Commissions & board members:** ✅ Complete. Migration 005, commission roster scraper, appointment extractor (Claude API), eSCRIBE discover-types, city config registry. 403 total tests. Plan: `docs/plans/2026-02-22-commissions-plan.md`. Remaining: commission meeting agenda scraping, conflict scanner extension for commission members, frontend commission pages.
...
- **GitHub Actions triple-trigger pattern:** `cloud-pipeline.yml` accepts `schedule` (weekly cron), `repository_dispatch` (n8n), and `workflow_dispatch` (manual). For cron runs, meeting date is auto-detected (next Tuesday). Input resolution uses `${{ github.event.inputs.X || github.event.client_payload.X }}` fallback. `data-sync.yml` uses the dual-trigger pattern (no cron — n8n handles sync scheduling).
- **`sync-pipeline.yml` is DEPRECATED.** It called the local `run_pipeline.py` which depends on local files. All production pipeline execution now goes through `cloud-pipeline.yml` which calls `cloud_pipeline.py` (Supabase-native). The deprecated workflow prints a warning and exits.
- **Production vs. development pipelines:** `cloud_pipeline.py` = production (reads/writes Supabase, used by GitHub Actions + n8n). `run_pipeline.py` = development (reads/writes local files, useful for testing extraction changes locally before deploying). Both remain in the repo.
- **Migration health check:** `/api/health` endpoint probes 18 tables across 5 migration groups (core_schema, 001_cloud_pipeline, 002_user_feedback, 003_nextrequest, 005_commissions). Returns healthy/degraded/unhealthy with 5-min cache. Python equivalent: `staleness_monitor.py --check` includes `check_schema_health()` which queries `information_schema.tables`.
...
- **Richmond has 30+ commissions and boards.** 17 currently seeded in `src/ground_truth/officials.json`. Major ones: Planning Commission, Rent Board, Design Review Board, Police Commission, Housing Authority, Human Rights & Human Relations Commission, Recreation & Parks Commission.
- **Commission roster pages use HTML tables.** `ci.richmond.ca.us/Boards` links to per-commission pages with `<table>` rows containing member name, appointing authority, and term dates. Pure `requests` + BeautifulSoup parsing — no Playwright needed.
- **Term date formats vary wildly.** Richmond uses at least 3 formats: "MM/DD/YYYY" (most common), "Month YYYY" (no day), "Pleasure of the Mayor" (indefinite). The roster scraper normalizes all to ISO dates or stores as-is for non-date terms.
- **Appointment extraction from council minutes.** Claude API (`tool_use` mode) extracts appointment actions from meeting text. Key patterns: "Motion by [member] to appoint [person] to [commission]", "reappointment of [person]", "resignation accepted from [person]". Returns structured JSON with action_type (appointment/reappointment/resignation/term_expiration), commission, member, vote, and term dates.
- **eSCRIBE discover-types for commission meetings.** `discover_meeting_types()` scans the eSCRIBE calendar API across a date range and returns all unique MeetingName values with counts and date ranges. Used to populate the `commissions_escribemeetings` registry in `city_config.py`. CLI: `python escribemeetings_scraper.py --discover-types`.
- **Commission meeting types in eSCRIBE.** Not all commissions have eSCRIBE pages. The 5 confirmed: "Planning Commission", "Richmond Rent Board", "Design Review Board", "Police Commission", "Housing Authority Board of Commissioners". Others meet less formally or use different portals.
- **Migration 005 numbering.** `005_commissions.sql` — numbered 005 (skipping 004) because 004 was reserved for a planned city-employees migration that hasn't been implemented yet. The migration is idempotent (IF NOT EXISTS throughout).
- **`commissions_escribemeetings` config pattern.** Maps canonical commission names (our internal names) to eSCRIBE MeetingName strings (what the API returns). This decouples our data model from platform-specific naming. Discovered via `--discover-types`.
...
### Multi-City Architecture (City Config Registry)

- **`src/city_config.py`** is the central registry. Each city is a dict keyed by FIPS code containing `name`, `state`, `fips_code`, `data_sources` (dict of source configs), and `council_members` (current + former sets).
- **Adding a new city:** Add a new entry to `CITY_CONFIGS` in `city_config.py` with the city's FIPS code as key. Each data source entry contains platform-specific fields (base URLs, API IDs, portal slugs). Only include sources the city actually has — the pipeline checks `if source in cfg["data_sources"]` before running.
- **Per-city data sources:** `escribemeetings` (base_url), `netfile` (agency_id, agency_shortcut), `nextrequest` (base_url, city_slug), `archive_center` (base_url, minutes_amid), `calaccess` (search_terms for Richmond-area filer matching), `socrata` (domain, datasets dict), `commissions_escribemeetings` (canonical name → eSCRIBE MeetingName mapping). Each source has different config fields because each platform's API is different.
- **Shared/global sources:** CAL-ACCESS is statewide (the bulk ZIP covers all California cities). The `calaccess` config entry contains city-specific search terms used to filter the statewide data.
- **`DEFAULT_FIPS = "0660620"`** appears in `cloud_pipeline.py`, `data_sync.py`, and `staleness_monitor.py` as the CLI default for backward compatibility. This keeps existing n8n workflows and GitHub Actions working without changes.
- **Config resolution pattern:** Entry-point functions call `get_city_config(city_fips)` to validate and resolve config. Raises `CityNotConfiguredError` for unknown FIPS codes. Config dict is then passed through to scraper/client functions.
- **Scraper refactoring pattern:** Each scraper gained a `resolve_config(city_fips=None)` function (or equivalent) that returns a config dict. When `city_fips` is provided, it reads from the registry; when `None`, it uses module-level defaults for backward compatibility with standalone CLI usage.
- **Conflict scanner council members:** `_get_council_members(city_fips=None)` loads current/former council member sets from the city config registry. Falls back to hardcoded defaults if FIPS is not provided or city is unknown (graceful degradation for standalone usage).
- **Testing:** `tests/test_city_config.py` (unit tests for registry functions), `tests/test_city_config_integration.py` (integration tests verifying pipeline config resolution), plus per-scraper `tests/test_*_city_config.py` files testing config resolution for each refactored module. 403 total tests (317 base + 86 from commissions work).
