...
- **Bias audit spec:** `docs/specs/bias-audit-spec.md` — instrumentation plan for detecting racial, gender, and demographic bias in the conflict scanner's matching logic. Identifies five structural bias surfaces (short-name thresholds, common-surname false positives, hyphenation handling, diacritics loss, Western-name regex). Defines two-level logging (`matching_decisions` table for individual flags + `scan_audit_summary` for per-meeting filter funnel stats), structural risk signals (compound surname, diacritics, name length, Census 2010 surname frequency tiers), pre-registered confidence thresholds, ground truth review workflow (interactive CLI with T/F/S/N verdicts + JSON sidecar fallback), and periodic bias audit process. **Fully implemented** — see entries below.
- **Census surname data pipeline:** `src/prepare_census_data.py` — downloads Census 2010 surname frequency data (162,254 surnames), assigns frequency tiers (Tier 1 = top 100, Tier 2 = 101–1000, Tier 3 = 1001–10000, Tier 4 = rest), outputs `src/data/census/surname_freq.json`. Used by `bias_signals.py` to tag each donor match with surname frequency tier. CLI: `python prepare_census_data.py [--skip-download]`.
- **Bias audit instrumentation (JSON sidecar):** `src/scan_audit.py` + `src/bias_signals.py` — every matching decision in `scan_meeting_json()` is logged with structural risk signals (compound surname, diacritics present, token count, char count, surname frequency tier). Per-scan aggregate statistics include filter funnel counts and surname tier distribution. JSON sidecar files saved to `src/data/audit_runs/` with UUID filenames. `ScanResult` gains `scan_run_id` and `audit_log` fields.
- **Audit sidecar persistence:** Both `run_pipeline.py` and `conflict_scanner.py` CLI now save audit sidecars to `src/data/audit_runs/` after each scan. Enables post-hoc analysis and ground truth review.
- **Surname tier tallying in scan loop:** Conflict scanner tallies surname frequency tier distribution for all contributions compared and for flagged contributions. Summary fields: `donors_surname_tier_{1,2,3,4}`, `donors_surname_unknown`, `flagged_surname_tier_{1,2,3,4}`, `flagged_surname_unknown`. Tested against real data: Feb 17 scan shows T1=4609, T2=5514, T3=6127, T4=6314, Unknown=3564 across 26,128 contributions.
- **Ground truth officials reference data:** `src/ground_truth/officials.json` — structured JSON of 7 current and 13 former Richmond council members with roles, districts, and notes. Used by ground truth review CLI to contextualize flags.
- **Ground truth review CLI:** `conflict_scanner.py --review --latest` or `--scan-run <UUID>` — interactive review of scanner flags. Presents each matched decision with donor, employer, agenda item context, match type, confidence, and bias signals. Reviewer assigns T (true positive), F (false positive), S (skip), or N (add notes). Verdicts saved to the audit sidecar with timestamps.
- **Periodic bias audit module:** `src/bias_audit.py` — reads all ground-truthed decisions across audit sidecars, computes per-tier false positive rates and per-name-property FP rates, flags disparities (warning if Tier 4 FP rate > 2× Tier 1). Pre-registered minimum of 100 ground-truthed decisions before reporting. CLI: `python bias_audit.py [--audit-dir PATH] [--min-decisions N] [--output PATH]`.
- **Comment format redesigned (three-tier publication):** `ConflictFlag.publication_tier` field: Tier 1 (≥0.6 confidence, sitting member) → public "Potential Conflicts" section; Tier 2 (≥0.4, sitting member) → public "Financial Connections" section; Tier 3 (low confidence, former candidates, PAC-only) → suppressed from comment, tracked internally. Template uses narrative prose, no raw confidence scores, methodology section with record counts. Verified on Feb 17, 2026 meeting: 4 flags (all Tier 3 internal-only), 25 clean items, comment ready for submission.
...
2. Accumulate 100+ ground-truthed flag decisions to run first periodic bias audit (use `--review --latest` after each pipeline run)
