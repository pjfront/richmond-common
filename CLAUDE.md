...
- `research/competitive-intel/` — **Negative map** of the civic-AI field. Per-company profiles for Locunity, Aware, citymeetings.nyc, Hamlet, Civic Sunlight, OpenCouncil, HeyGov, Next30Days, CivicPlus + `_self.md` (RC) + `_landscape.md` (market analysis). The point is to know what NOT to build toward — every other player operates under constraints (investors, buyers, runway) that RC doesn't have. Borrow their *technical* infrastructure freely; refuse their *business* infrastructure. The operator's structural moat is doing this for fun with no obligations to anyone. Start with [`research/competitive-intel/README.md`](docs/research/competitive-intel/README.md) for the matrix.
