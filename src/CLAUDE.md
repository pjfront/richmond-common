...
- **`data-sync.yml` scheduled jobs:** (1) **Daily** (7am UTC / 11pm Pacific): nextrequest only (CPRA has 10-day legal deadline). (2) **Weekly** (Mon 8am UTC / midnight Pacific): archive_center + minutes_extraction + netfile + nextrequest + socrata_expenditures + socrata_payroll. Each source is a separate step with `if: always()` so failures are isolated. (3) **Monthly** (1st at 9am UTC): calaccess full sync (1.5GB download, separate job). All sources also available via manual `workflow_dispatch`.
