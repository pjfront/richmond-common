...
- **GitHub Actions triggers:** `cloud-pipeline.yml` runs from its weekly/quarterly schedules or a manual `workflow_dispatch`. `data-sync.yml` also accepts `repository_dispatch` from the in-repo change detector, which uses the short-lived, run-scoped `GITHUB_TOKEN`.
