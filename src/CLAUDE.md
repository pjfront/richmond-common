...
- **`data-sync.yml` scheduled jobs:** (1) **Weekly** (Mon 7am UTC / Sun 11pm Pacific): archive_center + minutes_extraction + netfile + nextrequest + socrata_expenditures + socrata_payroll. Each source is a separate step with `if: always()` so failures are isolated. (2) **Monthly** (1st at 9am UTC): calaccess full sync (1.5GB download, separate job). All sources also available via manual `workflow_dispatch`.
