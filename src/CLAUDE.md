...
- change_detector includes a per-source budget field for traceability, but `data-sync.yml` distrusts that field and derives the enforced cap from its own source allowlist. Cloud-pipeline, post-meeting-recap, data-quality remain manual (workflow_dispatch only).
