...
- **`data-sync.yml` scheduled jobs:** (1) **Daily** (7am UTC / 11pm Pacific): nextrequest only (CPRA has 10-day legal deadline). (2) **Weekly** (Mon 8am UTC / midnight Pacific): archive_center + minutes_extraction + escribemeetings + netfile + nextrequest + socrata_expenditures + socrata_payroll. Each source is a separate step with `if: always()` so failures are isolated. (3) **Monthly** (15th at 9am UTC): calaccess (1.5GB download) + socrata_permits + socrata_licenses + socrata_code_cases + socrata_service_requests + socrata_projects. (4) **Quarterly** (1st of Jan/Apr/Jul/Oct at 10am UTC): form700 + form803_behested + lobbyist_registrations + propublica. All 17 sources also available via manual `workflow_dispatch`.
